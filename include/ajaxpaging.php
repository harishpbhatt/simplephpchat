...
///////////// NUMERIC FUNCTION WITH RECORD DESTAIL//////////////////////////////////////
	function number_pageing($query,$record_per_page='',$pages='',$detail='',$bottom='',$simple='',$cur_page="",$query_string="",$divid="")
	{
		...
		{ 
			...
			$this->tbl.="
			<li class='num_off' onclick=call_ajax_view('".$cur_page."','start=".($start-$this->record_per_page).$query_string."','".$divid."')>PREV</li>";
		...
		{
			$this->tbl.="<li class='num_on'>PREV</li>";
		} 
		...
		if($simple!='N')
		{
			...
			for($i=$start_loop;$i<$end_loop;$i++) 
			{
				if($current_page==$i)	
				{
					...
					$this->tbl.="<li class='num_on_active'>".$i."</li>";
				}	
				else 
				{ 
					...
					$this->tbl.="<li class='num_off' onclick=call_ajax_view('".$cur_page."','start=".($i-1)*$this->record_per_page.$query_string."','".$divid."')>".$i."</li>";
		...
		{ 
			...
			$this->tbl.="<li class='num_off' onclick=call_ajax_view('".$cur_page."','start=".($start+$this->record_per_page).$query_string."','".$divid."')>NEXT</li>";

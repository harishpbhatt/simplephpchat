...
///////////// NUMERIC FUNCTION WITH RECORD DESTAIL//////////////////////////////////////
	function number_pageing($query,$record_per_page='',$pages='',$detail='',$bottom='',$simple='',$cur_page="",$query_string="")
	{
		...
		{ 
			...
			if($listfile=="product images")
			{
			$this->tbl.="
			<li class='num_off' onclick=window.location.href='".$cur_page."?start=".($start-$this->record_per_page).$query_string."&p_id=".$_REQUEST['p_id']."'>PREVIOUS</li>";
			}else{
			$this->tbl.="
			<li class='num_off' onclick=window.location.href='".$cur_page."?start=".($start-$this->record_per_page).$query_string."'>PREVIOUS</li>";
			}
		...
		if($simple!='N')
		{
			...
			for($i=$start_loop;$i<$end_loop;$i++) 
			{
				...
				{ 
					...
					if($listfile=="product images")
			{
			$this->tbl.="<li class='num_off' onclick=window.location.href='".$cur_page."?start=".($i-1)*$this->record_per_page.$query_string."&p_id=".$_REQUEST['p_id']."'>".$i."</li>";
			}else{
					$this->tbl.="<li class='num_off' onclick=window.location.href='".$cur_page."?start=".($i-1)*$this->record_per_page.$query_string."'>".$i."</li>";
					}
		...
		{ 
			...
			if($listfile=="product images")
			{
			$this->tbl.="<li class='num_off' onclick=window.location.href='".$cur_page."?start=".($start+$this->record_per_page).$query_string."&p_id=".$_REQUEST['p_id']."'>NEXT</li>";
			}else{
			$this->tbl.="<li class='num_off' onclick=window.location.href='".$cur_page."?start=".($start+$this->record_per_page).$query_string."'>NEXT</li>";
			}
